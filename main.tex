...
compartments to be quantified.  We find that their areas and aspect ratios exhibit robust gamma distributions and exhibit a transition from a tight polygonal to a looser acircular packing geometry with stable eccentricity over time, evoking parallels and distinctions with the behavior of hydrated foams.  These results provide a quantitative benchmark for addressing
a general, open question in biology: How do cells collectively produce a complex structure external to themselves in a robust
and accurate manner? 
\end{abstract}
...
the dynamics of its structures alongside the positional configuration of the cells that secrete its components.
...
from $\sim 70\,\mu$m to $\sim 1\,$mm, raising the question of how the ECM geometry and relative cell positions transform during growth. 
...
We suggest that a detailed study of the geometry of a growing ECM made possible by this strain of \textit{Volvox}
will provide
...
it was added 24 hours before microscopy to increase PhII expression; 
...
power-law relationship with compartment area, which, in conjunction with the weak coupling between whitened offset and compartment size (E), further supports that conclusion in light of a scaling argument we explain in Discussion \S5.
...
Coordinate transforms are chosen in either linear- or log-scale, with natural offsets (e.g. $q_{\rm hex}$ in panel (C), the circularity of a regular hexagon), 
...
decreasing from stages I-IV, so the configuration (primarily the anterior hemisphere, \SI, Fig. S11) becomes increasingly disordered. The initial high 
...
as quantified in panel B, \S\ref{sec:cz3_ap}, and \S\ref{sec:cz3_aniso}. 
The metrics in B-C thus show the 
counterintuitive result that the CZ3 space partitioning 
is increasingly disordered as the
global sphericity is maintained (and even improved, panel F) during the dramatic enlargement.
...
\ref{table:growth}).
The surface areas of the compartments are well-approximated by gamma distributions (\eqref{eq:gamma}) 
...
in intrinsic structures of an ECM, with non-stationarity in $k$ revealing 
A/P differentiation. 
...
Both observations suggest a growth-induced deformation in which space is locally dilated (in tangent planes containing 
compartment surfaces) as $\R^2 \mapsto \rho \R^2, \rho \ge 1$, which indeed 
increases offsets while preserving whitened offsets (\SI, \S B.1).
Such transformations also preserve aspect ratios, consistent with earlier observations.
...
Such surface tension-adhesion trade-offs are known to result in acircular equilibria as packing fraction is decreased 
...
A better understanding of the self-organization of crosslinking ECM components will shed light 
