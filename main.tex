...
including the boundaries of compartments forming an epithelium-like architecture around somatic cells and the outer surface of the organism.
...
the joint evolution of ECM structures with the positional configuration of cells that secrete its components.
...
The ECM of \textit{V. carteri} has been studied in the past decades from the perspective 
of structure and 
...
from $\sim 70\,\mu$m to $\sim 1\,$mm, raising the question of how ECM geometry and relative cell positions evolve during growth. 
...
We suggest that a detailed study of 
growing ECM geometry made possible by this strain of \textit{Volvox}
will provide
...
To investigate this and to answer the more general questions raised above, we performed geometric analyses of the structures illuminated by localized PhII:YFP, using metrics (Table \ref{table:metrics}) which give insight into the coexistence of 
local geometric stochasticity at the cell scale and global  robustness at the organism scale during growth. 
From trends observed in the temporal evolution of these metrics, we find compelling physical analogues to the growth of ECM structures, including the dynamics of wet foams.
...
after such a short incubation with SIP, the females still show an unchanged cleavage program and 
...
Fig. \ref{fig8}A1-2 shows that the somatic cell area increases modestly, by $\sim 10\%$, along the PA axis at all stages. In contrast, the CZ3 compartment area grows substantially along this axis, with a minimum increase of $\sim 50\%$ in stage I and a maximum of $\sim 130\%$ in III and IV. 
Moreover, the slope increases after the equatorial region in all stages, most prominently in stage IV.
...
Perhaps counterintuitively, the cellular offset vector shows no correlation with the primary elongation axis of the compartment and its angle is uniformly distributed in $[0, \frac{\pi}{2}]$ in I-IV and S (\SI, Fig. S10).
...
The analysis above indicates compelling correlations between geometric features of the ECM during growth;
...
\cite{Day2022}, 
which is remarkable given the decreasing polygonality of CZ3 compartments observed in \S \ref{sec:r2v}.
...
We find the somatic CZ3 architecture becomes \textit{decreasingly} optimal during expansion\textemdash 
